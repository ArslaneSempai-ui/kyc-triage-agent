 ...
 */
export const REFERENTIEL_SECTORIEL = new Map([
    ["conseil", 165_000],
 ...
 */
export function netteteVolume(rapport, multiple = MULTIPLE_ANORMAL) {
    const net = Math.max(multiple + 0.5, 6);
    if (rapport >= net)
        return 0.95;
    if (rapport >= multiple)
        return 0.45 + ((rapport - multiple) / (net - multiple)) * 0.45;
    return 0.3;
}
/**
