 ...
 */
export const nombreRecu = (x) => (typeof x === "number" && Number.isFinite(x)) ? x : undefined;
export const booleenRecu = (x) => (typeof x === "boolean" ? x : undefined);
export const DECISIONS = new Set(["approuver", "complement", "escalader"]);
export const decisionRecue = (x) => (typeof x === "string" && DECISIONS.has(x)) ? x : undefined;
/**
...
const borner = (v) => Math.min(SEUIL_MAX, Math.max(SEUIL_MIN, v));
/**
...
function reproche(x) {
    ...
    if (Array.isArray(e.reprises)) {
        ...
        if (i !== -1)
            return `reprises[${i}] n'est pas une décision : ${JSON.stringify(e.reprises[i])}`;
    }
    return null;
}
let etat = vide();
