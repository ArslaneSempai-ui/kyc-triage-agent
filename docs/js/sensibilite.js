...
import { CONSTANTES } from "./agent.js";
/** The same table, every row multiplied by `facteur`. */
export function fausser(facteur) {
    const faussé = new Map();
    for (const [secteur, valeur] of REFERENTIEL_SECTORIEL)
        faussé.set(secteur, valeur * facteur);
    return faussé;
}
/**
...
export function tirages(combien = PAR_TIRAGE, graines = GRAINES) {
...
export function materiel(reference, obtenu) {
...
export function bande(reglage, jeux = tirages(), seuil = 0.7, k = CONSTANTES, pas = 30) {
     ...
     */
    const mesures = (v, avecTable = true) => jeux.map((cas) => mesurer(cas, seuil, avecTable ? REFERENTIEL_SECTORIEL : undefined, { ...k, [reglage]: v }));
    ...
    const bougentManquements = (v) => mesures(v).filter((b, i) => b.manquements !== base[i].manquements).length;
    const bougentEscalades = (v) => mesures(v).filter((b, i) => materiel(base[i].escaladesInutiles, b.escaladesInutiles)).length;
    ...
    const marcher = (vers, combienBougent) => {
        for (let i = 1; i <= pas; i++) {
            const v = valeur + ((vers - valeur) * i) / pas;
            const n = combienBougent(v);
            if (n > 0)
                return [valeur + ((vers - valeur) * (i - 1)) / pas, n];
        }
        return [vers, 0];
    };
    ...
    const accordExtreme = Math.max(pire.filter((d) => d !== 0).length, pireBas.filter((d) => d !== 0).length);
    ...
    const deManquements = [basM, hautM];
    const dEscalades = [basE, hautE];
    ...
    const dormant = [bas, haut, (bas + haut) / 2].some((v) => mesures(v, false).some((b, i) => b.manquements !== baseSans[i].manquements ||
...
export function bandes(jeux = tirages(), seuil = 0.7, k = CONSTANTES) {
    return Object.keys(PLAUSIBLE).map((r) => bande(r, jeux, seuil, k));
}
/** What to tell someone holding their own figure. */
export function conseil(b) {
    const f = (x) => (x < 100 ? x.toFixed(2) : Math.round(x).toLocaleString("en-GB"));
...
export function balayerErreur(facteurs = [0.7, 0.8, 0.9, 0.95, 1.0, 1.05, 1.1, 1.2, 1.3], seuil = 0.7, combien = 400) {
...
if (isMain(import.meta)) {
    ...
    for (const b of resultats) {
        ...
        console.log(`${b.reglage.padEnd(22)}${f(b.valeur).padStart(9)}${bande.padStart(21)}` +
    ...
    for (const p of balayerErreur()) {
        ...
        console.log(`${(signe + (p.erreur * 100).toFixed(0) + " %").padStart(13)}   ` +
